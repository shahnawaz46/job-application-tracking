...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "border border-border bg-input flex h-12 flex-row items-center justify-between gap-2 rounded-lg px-3 sm:h-11",
        Platform.select({
          web: cn(
            "w-fit whitespace-nowrap text-sm outline-none transition-colors duration-200",
          ...
          <TextClassContext.Provider value="text-popover-foreground">
            ...
            >
              <SelectPrimitive.Content
                className={cn(
                  "bg-popover border border-border relative z-50 min-w-[12rem] rounded-lg",
                  Platform.select({
                    web: cn(
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        "active:bg-accent group relative flex w-full flex-row items-center gap-2 rounded-md py-2.5 pl-2 pr-8 sm:py-2",
        Platform.select({
          web: "focus:bg-accent focus:text-accent-foreground cursor-default outline-none data-[disabled]:pointer-events-none *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 [&_svg]:pointer-events-none",
        }),
        props.disabled && "opacity-50",
        className,
      )}
